...
class Client():
    ...
    def send_text(self, content: str, receiver: str, aters: Optional[str] = ""):
        ...
        try:
            ...
            response = requests.post(self.base_url+'send_message_250514.html', headers=headers, data=data, timeout=30)
    ...
    def down_file(self, msg_id) -> str:
        ...
        def trigger_download_file(msg_id):
            ...
            try:
                ...
                response = requests.post(self.base_url+'trigger_download_file.html', headers=headers, data=data, timeout=30)
                response.raise_for_status()  # 如果响应状态码指示错误，将抛出HTTPError异常
                return response.content.decode('utf-8')  # 返回解码后的响应内容
            except requests.exceptions.RequestException as e:
                error_message = 'HTTP Request failed: {}'.format(e)
...
if __name__ == "__main__":
    ...
    r = c.send_image("http://b1.wcr222.top/0e2c4df62a691f11/2025/05/14/41658fc8b63e4172a4f10be967244210.jpg", 'yoin007')
    print(r)