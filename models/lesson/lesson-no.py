...
class Lesson:
    ...
    def _load_excel_file(
        ...
        sheet_name,
        index_col=None,
        error_msg="加载Excel文件失败",
    ):
        """加载Excel文件"""
        if not os.path.exists(file_path):
            self.log.error(f"{error_msg}: 文件不存在, {file_path}")
            return pd.DataFrame()
        try:
            ...
            self.log.error(f"{error_msg}: {str(e)}")
            return pd.DataFrame()
    ...
    def _load_class_template(self):
        ...
        return self._load_excel_file(
            template_path, "class", error_msg="加载班级模板失败"
        )

    def sorted_schedule_file(self, schedule_dir, monday):
        """按照文件名称的日期排序排课表文件"""
        schedule_files = []
        if not os.path.exists(schedule_dir):
            self.log.error(f"排课表文件夹不存在, {schedule_dir}")
        ...
        try:
            ...
            schedule_files = [
                f
                for f in files
                if f.startswith("课表") and f.endswith(".xlsx" and monday in f)
            ]
            schedule_files.sort(reverse=True)
            return schedule_files
        except Exception as e:
            self.log.error(f"获取排课表文件失败: {str(e)}")
            return schedule_files

    def current_schedule_file(self, week_next=False):
        """获取当前周的排课表文件"""
        if self._should_refresh_cache() and hasattr(self, "_last_refresh_time"):
            self.refresh_cache()
